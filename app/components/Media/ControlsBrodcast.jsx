			...
			<div className="control-container">
				<ul className="control-actions">
					<li className="control-action">
						<label className="control-action-item__timer-label">
							<i className="control-action-item__timer-icon" dangerouslySetInnerHTML={{__html: camera }}></i>
							<span>LIVE</span>
						</label>
					...
					<li className="control-action">
						<button onClick={() => this.toggleShare()} className="control-action-item__share-label withBorder">
							<i className="control-action-item__share-icon" dangerouslySetInnerHTML={{__html: share }}></i>
							<strong>Share</strong>
						</button>
					</li>
				</ul>
				...
				<Modal show={this.state.isShareOpen} onClose={this.toggleShare}>
						<i className="modal-icon" dangerouslySetInnerHTML={{__html: share }}></i>
						<h1>Share</h1>
						<div className="fieldset">
							<div className="legend">
								<u>Stream ID</u>
								<Clipboard copyLabel='Copy ID' value={broadcasting}></Clipboard>
							</div>
						...
						<div className="fieldset">
							<div className="legend">
								<u>Web Player</u>
								<Clipboard copyLabel='Copy url' value={playerURL}></Clipboard> <Openexternal class="withBorder withBorder-light" url={playerURL}></Openexternal>
							</div>
						</div>
				</Modal>
			</div>
